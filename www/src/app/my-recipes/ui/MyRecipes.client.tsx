...
export default function MyRecipesClient({
  initialLists = [] as RecipeList[],
}: {
  initialLists?: RecipeList[];
}) {
  const [lists, setLists] = useState<RecipeList[]>(initialLists);
  const [loading, setLoading] = useState<boolean>(initialLists.length === 0);
  ...
  const body = useMemo(() => {
    ...
    if (error)
      return <p style={{ color: 'var(--color-danger, #b00)' }}>{error}</p>;
              ...
              <>
                ...
                <ul className="list" style={{ maxWidth: 720 }}>
                  {(selected.items ?? []).map((it) => (
                    <li
                      key={it.recipeId}
                      className="list-item -no-gutters"
                    ...
                    >
                      ...
                      <span style={{ flex: 1 }}>
                        {it.recipe?.name ?? 'Receita'}
                      </span>
                      <button
                        onClick={() => handleRemoveRecipe(it.recipeId)}
                        style={{
                          padding: 8,
                          borderRadius: 6,
