  ...
  return (
    <section className="grid" aria-labelledby="food-bulk-title">
      <div className="g-col-12">
        <h2 className="section-title" id="food-bulk-title">Importar Alimentos (JSON)</h2>
      </div>

      <div className="g-col-12">
        <form onSubmit={onSubmit} style={{ display: 'grid', gap: 12 }}>
          <label htmlFor="food-bulk-text">Cole abaixo o JSON (array ou item único):</label>
          <textarea
            id="food-bulk-text"
            value={jsonText}
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {result && <span style={{ color: 'var(--green-7)' }}>Sucesso: {result}</span>}
            {error && <span style={{ color: 'var(--red-7)' }}>Erro: {error}</span>}
          </div>
        </form>
      </div>
    </section>
