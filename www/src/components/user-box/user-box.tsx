...
import { LanguageContext, type Language } from '@/contexts/language/language.context';
...
export const UserBox: FC<UserBoxProps> = ({ className = '', ...props }) => {
  ...
  const handleCredentialResponse = useCallback((credentialResponse: { credential?: string | null }) => {
  ...
  useEffect(() => {
    ...
    if (typeof window === 'undefined') return;
    const id = (window as any).google?.accounts?.id as GoogleAccountsIdNamespace | undefined;
    ...
    <>
      ...
      {user?.displayName && (
        <button className="action-button" onClick={handleLogout}>
          <span className="svg-icon"><ion-icon name="log-out-outline" /></span>
