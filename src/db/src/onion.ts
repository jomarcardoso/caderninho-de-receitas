/* eslint-disable */

export default {
  fdc_id: 515299,
  NDB_No: '515299',
  NDB_Number: 515299,
  NDB_Number_Legacy: null,
  typecode: 3,
  fndds_category_id: null,
  FdGrp_Order: null,
  FdGrp_Desc: null,
  type: null,
  type2: 'Baked Foods',
  commontype: null,
  name: 'Blazing Bagels And Bakery Inc. - Onion',
  name1: 'Onion',
  name2: null,
  name3: 'Blazing Bagels And Bakery Inc. - Onion',
  servingsizes:
    '{"wt1":[128,"1 bagel"],"oz":[28.35,"1 oz"],"200cals":[69.204,"200 calorie serving"],"100g":[100,"100 grams"]}',
  brand_owner: 'Blazing Bagels And Bakery Inc.',
  gtin_upc: '652003101262',
  ingredients:
    "Shepherd's Grain Unbleached Enriched Flour (Wheat Flour, Malted Barley Flour, Niacin, Reduced Iron, Thiamin Mononitrate, Riboflavin, Folic Acid), Water, Brown Cane Sugar (Sugar, Invert Sugar, Cane Molasses), Malted Barley Flour, Dehydrated Onion, Contains 2% Of Each Of The Following: Sea Salt, Corn Meal, Yeast, Calcium Sulfate, Enzymes (Plant Based).",
  serving_size: '128',
  serving_size_unit: 'g',
  household_serving_fulltext: '1 BAGEL',
  branded_food_category: 'Breads & Buns',
  data_source: 'LI',
  modified_date: '2018-06-06',
  available_date: '2019-04-01',
  Pro_Factor: null,
  Fat_Factor: null,
  CHO_Factor: null,
  data_type: 'branded_food',
  data_db_name: 'Branded Food',
  db_sort: 4,
  food_description: 'ONION',
  category_id: '',
  publication_date: '2019-04-01',
  FORMY10: null,
  F10D0: null,
  F110: null,
  F12D0: null,
  F121: null,
  F13D0: null,
  F14D0: null,
  F14D1: null,
  F141c: null,
  F141T: null,
  F15D0: null,
  F15D1: null,
  F16D0: null,
  F16D1: null,
  F16D1C: null,
  F16D1T: null,
  F17D0: null,
  F17D1: null,
  F171c: null,
  F18D0: null,
  F18D1: null,
  F18D1C: null,
  F18D1T: null,
  F18D1TN7: null,
  F18D2: null,
  F182c: null,
  F18D2CLA: null,
  F182TI: null,
  F18D2CN6: null,
  F182T: null,
  F18D2TT: null,
  F18D3: null,
  F183c: null,
  F18D3CN3: null,
  F18D3CN6: null,
  F183t: null,
  F183I: null,
  F18D4: null,
  F20D0: null,
  F20D1: null,
  F201c: null,
  F201t: null,
  F202c: null,
  F20D2CN6: null,
  F20D3: null,
  F203c: null,
  F20D3N3: null,
  F20D3N6: null,
  F203np: null,
  F20D4: null,
  F204c: null,
  F20D4N6: null,
  F205c: null,
  F20D5: null,
  F210: null,
  F21D5: null,
  F22D0: null,
  F22D1: null,
  F22D1C: null,
  F22D1CN11: null,
  F22D1CN9: null,
  F22D1T: null,
  F222: null,
  F223: null,
  F22D4: null,
  F225c: null,
  F22D5: null,
  F226c: null,
  F22D6: null,
  F230: null,
  F24D0: null,
  F24D1C: null,
  HYDRO25: null,
  F4D0: null,
  FORMY: null,
  FORMYTET: null,
  F50: null,
  F6D0: null,
  F70: null,
  F8D0: null,
  F90: null,
  ACETIC: null,
  ALA_G: null,
  ALC: null,
  ARG_G: null,
  ASH: null,
  ASP_G: null,
  BSISTR: null,
  SITSTR: null,
  BETN: null,
  BIOTIN: null,
  BORON: null,
  BRASS: null,
  CAFFN: null,
  CA: 16,
  CAMNL: null,
  CAMD5: null,
  CHOCDF: 59.38,
  CHOCDF_SUM: null,
  OTHER_CARB: null,
  CARTA: null,
  CARTB: null,
  CL: null,
  CHOLE: 0,
  CHLNF: null,
  CHLNGP: null,
  CHLNP: null,
  CHLNPC: null,
  CHLNSP: null,
  CHOLN: null,
  CR: null,
  CARTB_CIS: null,
  LUTZEA_CIS: null,
  LYCPN_CIS: null,
  COBALT: null,
  CU: null,
  ACRYPX: null,
  CRYPX: null,
  CYSTEINE: null,
  CYS_G: null,
  VITK1D: null,
  ENERC_KCAL: 289,
  ENERC_KJ: null,
  EDIGAL: null,
  FAMS: null,
  FAPU: null,
  FASAT: 0,
  FATRN: 0,
  TTDFAT: null,
  FATRNM: null,
  FATRNP: null,
  INSOL_FIBER: null,
  SOL_FIBER: null,
  FIBTG: 2.3,
  FLD: null,
  FOLDFE: null,
  FOLFD: null,
  FOL: null,
  FOLAC: null,
  FRUS: null,
  GALS: null,
  GLUS: null,
  GLU_G: null,
  GLUTAM: null,
  GLY_G: null,
  HISTN_G: null,
  HYP: null,
  INO: null,
  INU: null,
  IODINE: null,
  FE: 3.52,
  ILE_G: null,
  LACTIC: null,
  LACS: null,
  LEU_G: null,
  LUT: null,
  LUTZEA: null,
  LYCPN: null,
  LYS_G: null,
  MG: null,
  MALS: null,
  MN: null,
  MK4: null,
  MET_G: null,
  MO: null,
  NIA: 4.688,
  NI: null,
  NTRGN: null,
  PANTAC: null,
  PHE_G: null,
  P: null,
  PHYTO: null,
  PHYTOF: null,
  PHYSTR: null,
  K: null,
  PRO_G: null,
  PROCNT: 10.94,
  RETOL: null,
  RIBF: 0.332,
  RIBOSE: null,
  SE: null,
  SER_G: null,
  NA: 430,
  SORB: null,
  GRAVITY: null,
  STARCH: null,
  STID7: null,
  SUCS: null,
  ADD_SG: null,
  SUGAR: 7.03,
  SUGAR_NLEA: null,
  SULFUR: null,
  TAURINE: null,
  THEBRN: null,
  THIA: 1,
  THR_G: null,
  TOCPHB: null,
  TOCPHD: null,
  TOCPHG: null,
  TOCTRA: null,
  TOCTRB: null,
  TOCTRD: null,
  TOCTRG: null,
  TOTAL_FAT: null,
  FAT: 0.78,
  SUGAR_ALC: null,
  CARTB_TRANS: null,
  LYCPN_TRANS: null,
  TRP_G: null,
  TYR_G: null,
  VAL_G: null,
  VITA_IU: 78,
  VITA_RAE: null,
  VITB12: null,
  VITB12_ADD: null,
  VITB6A: null,
  VITC: 0,
  VITD_IU: null,
  VITD: null,
  ERGCAL: null,
  CHOCAL: null,
  VIT_E_ATE: null,
  TOCPHA: null,
  VIT_E_LABEL: null,
  VITE_ADD: null,
  VITK1: null,
  WATER: 28.9,
  XYLITOL: null,
  ZEA: null,
  ZN: null,
  Omega3: null,
  Omega6: null,
  NetCarb: 57.08,
  Om3Om6Ratio: null,
  Om6Om3Ratio: null,
  common: 0,
  GmWt_1: 128,
  GmWt_2: null,
  GmWt_3: null,
  GmWt_4: null,
  GmWt_5: null,
  GmWt_6: null,
  GmWt_7: null,
  GmWt_8: null,
  GmWt_9: null,
  Gm_Wt200Cals: 69.204,
  GmWt_Desc1: '1 bagel',
  GmWt_Desc2: '',
  GmWt_Desc3: '',
  GmWt_Desc4: '',
  GmWt_Desc5: '',
  GmWt_Desc6: '',
  GmWt_Desc7: '',
  GmWt_Desc8: null,
  GmWt_Desc9: null,
  vegetarian: null,
  vegan: null,
  url: null,
  amino: null,
  commonserving: 'wt1',
  commondesc: '1 bagel',
  commonweight: 128,
  PRAL: null,
  legacy: 0,
  eliminated: 0,
  CITRIC: null,
  MALIC: null,
  d5a: null,
  Otherphy: null,
  TOTFIBER_AOAC: null,
  new: 1,
  UID: null,
  user_ingredients: null,
} as any;
