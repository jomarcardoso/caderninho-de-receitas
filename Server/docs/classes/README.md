...
- `GET /api/food/search` — search foods (summary)

  - Query: `text` (string, required), `categories` (csv, optional), `limit` (int, default 25, max 64)
  - Response: [`FoodSummaryResponse`](#foodsummaryresponse)[]
...
- `GET /api/food/icons` — icon lookup
  ...
  - Auth: AdminOrHigher
  - Response: [`Icon`](#icon)[] (or summary with `id`, `name`, `url`)
